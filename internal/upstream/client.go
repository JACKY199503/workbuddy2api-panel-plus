...
func Classify(status int, body string) ErrKind {
	...
	for _, m := range accountFaultMarkers {
		...
		// 14018「Credits exhausted」：上游账号积分耗尽（msg 指向购买加油包）。
	// 判定**先于** status==429 —— 上游该形态返回的就是 429 状态码，若按状态码先判
	// 会被归成 ErrSoftRate（hint "retry after reset"），而「没钱」在限流窗口内不会
	// 自愈，客户端据此重试必然再失败（且放大风控）。14018 是该语义的确定业务码，
	// 精确匹配；不并入 hardMarkers 宽词表（429 body 高频携带 "quota exceeded" 等跨
	// 计费/限流两界的措辞，宽词先判会把真限流误归硬冷却，白扔号约 12h）。
	if codeMarker(lower, "14018") {
		return ErrHardCredit
	}
