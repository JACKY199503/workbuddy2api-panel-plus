...
# REALM_OK 各域可服务性（/healthz realm_servable）：不可用的域，其上所有模型的
# 上游失败记 SKIP（环境性），不计 FAIL。
REALM_OK = {}
try:
    _r = urllib.request.urlopen(urllib.request.Request(BASE + "/healthz"), timeout=10)
    REALM_OK = json.loads(_r.read().decode("utf-8")).get("realm_servable") or {}
except Exception:
    pass
...
def _stream_err(status, raw):
    """流式请求返回非 200：错误体按协议形态判定（710 兼容：上游不可用 ≠ 协议错误）。"""
    v, det = upstream_or_fail(status, raw, "error")
    if v == "UPSTREAM":
        return "UPSTREAM", det + " (stream)"
    return "FAIL", "http %s wrong error shape" % status


def verdict_chat_stream(status, events, ct, raw=""):
    if status != 200:
        return _stream_err(status, raw)
...
def verdict_responses_stream(status, events, ct, raw=""):
    if status != 200:
        return _stream_err(status, raw)
    if "text/event-stream" not in ct:
        return "FAIL", "ct=%s" % ct
...
def verdict_messages_stream(status, events, ct, raw=""):
    if status != 200:
        return _stream_err(status, raw)
    if "text/event-stream" not in ct:
        return "FAIL", "ct=%s" % ct
...
def case_model(m, do_stream=True):
    ...
    for attempt in range(RETRY_EMPTY + 1):
        ...
        if do_stream:
            ...
            out.append(("chat", "stream", m) + verdict_chat_stream(st, ev, ct, ct))
            st, ev, ct = http_stream("/v1/responses", {"model": m, "input": PROMPT, "max_output_tokens": MAXTOK, "stream": True})
            out.append(("responses", "stream", m) + verdict_responses_stream(st, ev, ct, ct))
            st, ev, ct = http_stream("/v1/messages", {"model": m, "messages": [{"role": "user", "content": PROMPT}], "max_tokens": MAXTOK, "stream": True})
            out.append(("messages", "stream", m) + verdict_messages_stream(st, ev, ct, ct))
        if not any(r[3] == "WARN" for r in out):
            break
        if attempt < RETRY_EMPTY:
            time.sleep(2)
    # 域不可用（该 realm 无任何可用账号，例如 global 号被人工停用）时的上游失败
    # 一律记 SKIP：这是环境性不可用，不是协议层问题（/healthz realm_servable 同源判据）。
    realm = m.split(":")[0] if ":" in m else "cn"
    if REALM_OK.get(realm) is False:
        out = [r[:3] + ("SKIP", "realm %s unavailable (no healthy account)" % realm) if r[3] in ("UPSTREAM", "FAIL") else r for r in out]
    return out
