...
> ## 🧩 这是二次修改版（fork of fork）
>
> | 层 | 项目 | 许可 |
> |---|---|---|
> | 根项目 | [Sliverkiss/workbuddy2api](https://github.com/Sliverkiss/workbuddy2api) | MIT |
> | 直接上游 | [linguo2625469/workbuddy2api-panel](https://github.com/linguo2625469/workbuddy2api-panel)（Web 面板增强分支） | MIT |
> | **本仓库** | 在上游之上**只新增两项功能** | MIT |
>
> 新增的两项功能（本文有专章说明）：
...
> **本仓库提交的源码 = 上游功能 + 二改的两项增强，已经合并好了。**
> 不需要先装上游、也不需要先打补丁 —— `git clone` 或 `docker pull` 直接就是带 **API 密钥分发** 与 **模型编排** 的完整网关 + 面板。
...
- 镜像：`ghcr.io/JACKY199503/workbuddy2api-panel-plus:latest`（linux/amd64，push 到 main 时自动构建）。
- 想自己编译源码：把 `docker-compose.yml` 里 `image:` 注释掉、放开 `build: .`，或 `docker build -t wb2api .` —— 编出来的二进制同样自带两项增强。
...
| `bash tools/check.sh` | 运维自检：容器 / 健康 / 账号 / 快照 / 最近日志 |
| `bash tools/check-enhancements.sh` | **两项增强的 16 个注入点自检**（合并上游后确认增强没被冲掉） |
| `bash tools/snapshot.sh` / `tools/restore.sh` | 源码快照与回滚（升级前拍一张） |

改过 `internal/apikeys/`、`internal/autoroute/`、`internal/panel/keys.go` 或合并过上游后，跑 `tools/check-enhancements.sh` + [tests/](tests) 的回归脚本，判据见 [docs/ACCEPTANCE.md](docs/ACCEPTANCE.md)。
...
- 本仓库二改（2026-09）：**API 密钥分发** 与 **模型编排**，对应新增代码：
