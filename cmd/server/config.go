...
type Config struct {
	...
	// TrustProxy 是否信任反向代理的 X-Forwarded-For / X-Real-IP。
	// 默认 false：直连部署下这些头可被任意伪造，信任它等于 IP 白名单可绕过。
	...
	// AutoModel 模型编排：auto 虚拟模型按昼夜时段挑主模型 + 降级链。
	//
	// 客户端写 model=auto（或 cn:auto / global:auto），网关按当前 Asia/Shanghai
	// 小时在 [day_start, day_end) 内用 day_primary、否则用 night_primary；失败时
	// 按 fallback 顺序降级。详见 internal/autoroute。
	AutoModel struct {
		// Enabled 编排开关。false（默认）= auto 不是虚拟模型，按普通模型名直出
		// （未配置时零回归）。
		Enabled bool `json:"enabled"`
		// DayPrimary / NightPrimary 白天/夜间主模型（带 realm 前缀，如 cn:hy3）。
		// 留空 = 该时段无主模型（回落到对方；都空则从 fallback 里挑第一个符合
		// realm 的）。
		DayPrimary   string `json:"day_primary"`
		NightPrimary string `json:"night_primary"`
		// DayStart / DayEnd 白天窗口 [start, end)，默认 8 / 23（08:00–23:00 白天）。
		DayStart int `json:"day_start"`
		DayEnd   int `json:"day_end"`
		// VirtualID 虚拟模型名，默认 "auto"。上游可能已下发同名真实模型
		// （上游 cn 侧确实有过 "auto"）：默认让位不接管，改个名（如 auto-router）
		// 或开 Override 才会接管。
		VirtualID string `json:"virtual_id"`
		// Override 虚拟模型名与上游真实模型同名时仍接管（覆盖上游同名模型语义）。
		Override bool `json:"override"`
		// Fallback auto 的降级链（有序，带 realm 前缀）。
		Fallback []string `json:"fallback"`
		// OnEmpty 上游 200 但正文为空时视为失败并降级（issue #31：部分模型默认
		// reasoning_effort=max 吃满 max_tokens 预算 → 空回复不报错）。默认 true；
		// 仅当候选链长度 > 1 时才会真的降级，单模型场景零行为变化。
		OnEmpty bool `json:"on_empty"`
		// FallbackOn 可触发降级的错误类别名；空 = 内置默认集合
		// （soft_rate / hard_credit / model_blocked / server / account_fault /
		// not_found / no_healthy_account）。
		FallbackOn []string `json:"fallback_on"`
	} `json:"auto_model"`

	// ModelFallback 任意模型 → 降级链（有序）。键是客户端写的模型名（含 realm
	// 前缀，与请求里的 model 字段逐字匹配）；值是候选模型名列表。
	// 与 auto_model.fallback 的关系：前者是「auto 虚拟模型」的链，这里是「具名
	// 模型」的链；两条链可叠加（链上每项再按本表递归展开，限深 3、去重）。
...
func Default() *Config {
	...
	// 模型编排默认关闭（未配置时 auto 不是虚拟模型，零回归）；窗口默认 08:00–23:00
	// 白天（对齐上游夜间免费窗口的常见作息）。启用需显式 enabled=true。
	c.AutoModel.Enabled = false
	c.AutoModel.DayStart = 8
	c.AutoModel.DayEnd = 23
	// 空回复降级默认开（单模型链不会触发任何行为变化，多模型链可避免空回复）。
	c.AutoModel.OnEmpty = true
	return c
}

// Auto 把配置段转成编排层配置（autoroute）。空项在此归一（去空、trim）。
func (c *Config) Auto() autoroute.Config {
	fb := make([]string, 0, len(c.AutoModel.Fallback))
	for _, m := range c.AutoModel.Fallback {
		if s := strings.TrimSpace(m); s != "" {
			fb = append(fb, s)
		}
	}
	mf := make(map[string][]string, len(c.ModelFallback))
	for k, v := range c.ModelFallback {
	...
	return autoroute.Config{
		Enabled:       c.AutoModel.Enabled,
		VirtualID:     strings.TrimSpace(c.AutoModel.VirtualID),
		Override:      c.AutoModel.Override,
		DayPrimary:    strings.TrimSpace(c.AutoModel.DayPrimary),
		NightPrimary:  strings.TrimSpace(c.AutoModel.NightPrimary),
		DayStart:      c.AutoModel.DayStart,
		DayEnd:        c.AutoModel.DayEnd,
		Fallback:      fb,
		ModelFallback: mf,
		OnEmpty:       c.AutoModel.OnEmpty,
		FallbackOn:    c.AutoModel.FallbackOn,
	}
}
...
// validateAutoModel 校验模型编排配置（auto_model 窗口 + 主模型名）。
//
// 只对「显式启用」做严格校验：未启用时用户留下的半截配置不阻塞启动（老配置零
// 影响），启用时才要求主模型非空。小时非法一律 fail fast（同 schedule 的口径）。
func (c *Config) validateAutoModel() error {
	if !c.AutoModel.Enabled {
		return nil
	}
	s, e := c.AutoModel.DayStart, c.AutoModel.DayEnd
	if s < 0 || s > 23 || e < 0 || e > 23 {
		return fmt.Errorf("auto_model.day_start/day_end: %d/%d 不是合法小时（0-23）", s, e)
	}
	day := strings.TrimSpace(c.AutoModel.DayPrimary)
	night := strings.TrimSpace(c.AutoModel.NightPrimary)
	if day == "" && night == "" && len(c.AutoModel.Fallback) == 0 {
		return fmt.Errorf("auto_model.enabled=true 但 day_primary / night_primary / fallback 全为空：至少需要给一个候选模型（如 \"cn:hy3\"）")
	}
	return nil
}
